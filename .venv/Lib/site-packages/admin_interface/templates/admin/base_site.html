...
{% block title %}
    ...
    {% if title %}{{ title }} | {% endif %}{% if theme.title %}{% trans theme.title %}{% else %}
        {{ site_title|default:_('Django administration') }}{% endif %}
{% endblock %}
...
{% block bodyclass %}admin-interface flat-theme
    {% get_admin_interface_theme as theme %}{% if theme.name %}{{ theme.name|slugify }}-theme{% endif %}{% endblock %}
...
{% block branding %}
    ...
    <h1 id="site-name">
        ...
        {% if theme.title_visible %}
            <span>{% if theme.title %}{% trans theme.title %}{% else %}
                {% trans 'Django administration' %}{% endif %}</span>
        {% endif %}
    </h1>
{% endblock %}
...
{% block welcome-msg %}
    ...
    {% endif %}
    {% if theme.env_visible_in_header %}<span class="environment-label {{ theme.env_name }}"></span>
        - {% endif %}{{ block.super }}<br>
{% endblock %}
